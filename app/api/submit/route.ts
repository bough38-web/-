...
export async function POST(req: Request) {
  try {
    ...
    if (!sbUrl || !sbUrl.startsWith('https://')) {
      return NextResponse.json({ error: `설정 오류: Supabase URL이 올바르지 않습니다. (현재값 앞부분: ${sbUrl?.substring(0, 8)}...)` }, { status: 500 });
    }
    if (!sbKey || !sbKey.startsWith('ey')) {
      return NextResponse.json({ error: `설정 오류: Service Role Key가 올바르지 않습니다. (현재값 앞부분: ${sbKey?.substring(0, 5)}...)` }, { status: 500 });
    }
    // -------------------------
